...
author: "Weiqiao Shen & Raylynn Zhou"
output: pdf_document
---
...
```{r bootstrap without imputation}
# 
# set.seed(1)
# nsim <- 1000
# 
# # adjusted
# boot_OR_mi_noimp <- suppressWarnings(replicate(nsim, tmp.one.boot(dat.mi, variables = c("agebin", "HER2FISH", "TOP2Atri"))))
# boot_OR_rfi_noimp <- suppressWarnings(replicate(nsim, tmp.one.boot(dat.rfi, variables = c("agebin", "HER2FISH", "TOP2Atri"))))
# all_boot_OR_noimp  <- as.data.frame(cbind(t(boot_OR_mi_noimp), t(boot_OR_mi_noimp)))
# names(all_boot_OR_noimp) <- names(all_perm_OR)
# 
# # crude
# boot_OR_mi_simple_noimp <- suppressWarnings(replicate(nsim, tmp.one.boot(dat.mi)))
# boot_OR_rfi_simple_noimp <- suppressWarnings(replicate(nsim, tmp.one.boot(dat.rfi)))
# all_boot_OR_simple_noimp <- as.data.frame(cbind(t(boot_OR_mi_simple_noimp), t(boot_OR_rfi_simple_noimp)))
# names(all_boot_OR_simple_noimp) <- names(all_perm_OR_simple)
# 
# 
# # visualizations
# # adj, amp
# ggplot() + 
#   geom_density(data = all_boot_OR_noimp, aes(x = log(TOP2Atri1_mi), y = ..density.., color = "mi")) + 
#   geom_density(data = all_boot_OR_noimp, aes(x = log(TOP2Atri1_rfi), y = ..density.., color = "rfi")) + 
#   geom_vline(aes(xintercept = (get.CI(all_boot_OR_noimp$TOP2Atri1_mi, 
#                                       actual_OR = actual.OR.mi[5])), 
#                  color = 'miCI')) +
#   geom_vline(aes(xintercept = (get.CI(all_boot_OR_noimp$TOP2Atri1_rfi, 
#                                       actual_OR = actual.OR.rfi[5])), color = 'rfiCI')) +
#   labs(x = "Crude log OR between amplification and normal TOP2A",
#        title = "Bootstrapping Crude log OR with Imputations, Amplification") +
#   scale_color_manual(name='Legend',
#                      values=c('mi'='darkgreen', 'rfi'='darkblue', 'miCI' = 'orange', 'rfiCI' = 'darkred'))
